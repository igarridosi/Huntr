...
async function main() {
  ...
  console.log(ticker, 'existing rows', existingPayload ? (existingPayload.rows || []).length : 0);

  try {
    const pythonBin = process.env.PYTHON_BIN || 'python';
    const out = execFileSync(pythonBin, ['scripts/yfinance-quarterly-metrics.py', '--ticker', ticker, '--limit', '16'], {
      encoding: 'utf8',
