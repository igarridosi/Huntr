...
- Chart Builder: compare up to four companies on any of 49 metrics, or mix metrics of one company on two axes, with periods aligned by calendar quarter and up to twenty years of statements, on a quarterly, trailing-twelve-month or annual basis (a TTM margin is the ratio of the four-quarter sums, not an average of margins). Every amount can be shown as reported, per share, year-over-year growth or indexed to the start of the window; prices as % change on a linear or log axis, with statement bars standing on the fiscal close they report. Nine templates, a period slider, four canvas themes, value labels that stay legible, a legend that reads values out on hover, saved charts, a shareable link and PNG export with the watermark. Statements before a company's listing are left out, so a chart never describes a market history the stock did not have.
- Opportunity radar: unusual volume, buybacks, 52-week breakouts, yield leaders.
- Tracks: what management does and says. Insiders come from each company's Form 4 filings on SEC EDGAR, and only open-market trades outside a Rule 10b5-1 plan count as an insider buying or selling — grants, exercises, tax withholding, gifts and scheduled plan trades are shown but kept out of the totals. Twelve months summarised, the trades on the price chart, every line with its accession number and footnotes, a flag when three or more insiders buy within 90 days, a card on each ticker page and a feed across the watchlist. Transcripts join the same section once an official source is in place.
...
494 tests across 31 files, running in about two seconds with no network access. They cover the DCF engine, the Monte Carlo sampler, the reverse DCF solver, net-debt integrity across scenarios, SEC EDGAR extraction (debt cascades, restricted cash, share-count selection), currency and data-quality guards, scenario coherence, the cache guards that stop an empty response from being stored, the Chart Builder's spec model, resolver and templates (calendar alignment, the trailing-twelve-month period and growth transforms, indexing against the period before the window, pre-listing clipping), the Alpha Vantage statement mappers and the repair of cached bundles, and the live price stamped on every DCF scenario before export.
...
- Product analytics are first-party and anonymous. Page views and a short list of product events (a valuation shown or withheld, an export, a saved chart) are written from a Server Action into `analytics_events`, keyed on a random id in a 30-day httpOnly cookie. No IP address, user agent, referrer or fingerprint is stored, query strings are cut off paths before they are saved, and the table has RLS on with no policy, so only the service role reads it. The numbers are read back through SQL aggregates on `/app/admin/analytics`, which 404s for anyone not listed in `ANALYTICS_ADMIN_IDS`.
