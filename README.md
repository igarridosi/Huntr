...
386 tests across 20 files, running in about two seconds with no network access. They cover the DCF engine, the Monte Carlo sampler, the reverse DCF solver, net-debt integrity across scenarios, SEC EDGAR extraction (debt cascades, restricted cash, share-count selection), currency and data-quality guards, scenario coherence, the cache guards that stop an empty response from being stored, the Chart Builder's spec model, resolver and templates (calendar alignment, TTM and growth transforms, indexing against the period before the window, pre-listing clipping), and the Alpha Vantage statement mappers.
